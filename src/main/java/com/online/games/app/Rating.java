...
public class Rating {
    ...
    public void run(Scanner scanner, MongoDatabase database){
                        ...
                        while (!sub_exit) {
                            ...
                            System.out.println("2: Delete All ratings by user");
                            System.out.println("3: List All ratings");
                            System.out.println("4: List All ratings by user or game");
    ...
    private void readByDate(MongoDatabase database, String date, int skipDocuments, int pageSize) {
        FindIterable<Document> page = database.getCollection("ratings").find(eq("date", date)).skip(skipDocuments).limit(pageSize);
        for (Document p : page) {
            Object id = p.get("_id");
